...
for i, sl in enumerate(subset_slices):
    ...
    # TOF
    if use_tof:
        subset_lm_proj.tof_parameters = parallelproj.TOFParameters(
...
# img_np = np.flip(img_np, axis=0)  # Flip along the x-axis, commented out
img_np = np.flip(img_np, axis=1)  # Flip along the y-axis
img_np = np.flip(img_np, axis=2)  # Flip along the z-axis
nib.save(
    nib.Nifti1Image(img_np, affine=np.eye(4)),
    "new3testcylinder3_norm_test.nii.gz"
)
