...
parser.add_argument("-p", "--path", type=str, default="aug6flange/output",
                    help="path prefix for input files")
parser.add_argument("-o", "--output", type=str, default="estimations.csv",
                    help="output file name")
parser.add_argument("-t", "--time", type=float, default=10.0,
                    help="total simulation time in seconds")
parser.add_argument("-l", "--list", action="store_true",
                    help="store contamination list")
...
if __name__ == "__main__":
    ...
    for i in FILE_RANGE:
        ...
        singles_count, prompts_count, coin_lor, dw_nums, actuals = randoms.read_file(
            infile, TAU, TIME, DELAY, DETECTORS_SIM
        )
        ...
        print(f"File {str(i)} processed. SP: {sp[-1]}, DW: {dw[-1]}, "
              f"SR: {sr[-1]}, Actual: {actual[-1]}, Total: {total[-1]}")
        assert False

    if not CONT_LIST:
        # store the collected data
        df = pd.DataFrame({'sp': sp, 'dw': dw, 'sr': sr, 'actual': actual,
                           'multis': multis, 'total': total})
