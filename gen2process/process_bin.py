...
FILE_PREFIX = '/scratch/users/eeganr/realpointsource/Na22_250uCi_Single_06212024_minus5_PETA'
FILE_SUFFIX = '_ch0_C_PCIe.dat'

parser = argparse.ArgumentParser(description='Parameters')
parser.add_argument('n', help='number to process')
args = parser.parse_args()

filename = f"{FILE_PREFIX}{args.n}{FILE_SUFFIX}"
if not os.path.exists(filename):
    raise FileNotFoundError(f"File {filename} does not exist.")
else:
    print("Found file!")
event = getSingleArray(filename)
print("Event array created.")
with open(f"singles_{args.n}.pkl", 'wb') as f:
    pickle.dump(event, f)