...
then
  ...
  cd /home/users/nasir909/finely/Nasir_PhytoPET
...
if [ "$MODE" = "NORM" ]
then
  OUTPUT_FILE="/home/users/nasir909/finely/PhytoPET_Simulation/${BASE_FILE}${i}"
  echo $CAMERA_FILE
  echo $SOURCE_FILE
  echo $OUTPUT_FILE
  $GATE -a [randomseed,$RANDOM_SEED][filename,$OUTPUT_FILE][camerafile,$CAMERA_FILE][sourcefile,$SOURCE_FILE][timestart,$TSTART][timeslice,$TSLICE][timestop,$TSTOP] simu_pet.mac > /home/users/fwdesai/Gen2/data/train1_normr$i.out
  echo $RANDOM_SEED
#  RANDOM_SEED=`expr $RANDOM_SEED + $INCR_SEED`
fi
...
####################################################
  ...
  echo $OUTPUT_FILE
  echo $CAMERA_FILE
  $GATE -a [randomseed,$RANDOM_SEED][filename,$OUTPUT_FILE][camerafile,$CAMERA_FILE][sourcefile,$SOURCE_FILE][timestart,$TSTART][timeslice,$TSLICE][timestop,$TSTOP] simu_pet.mac > /scratch/users/fwdesai/PhytoPET/spatialres/train1_r$i.out
  echo $RANDOM_SEED
